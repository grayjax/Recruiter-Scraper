...
async def process_page_via_navigation(page: Page, title_whitelist: set | None = None) -> list[dict]:
    """
    ...
    try:
        ...
        panel_data = await page.evaluate("""
            () => {
                ...
                // ── Location: Same dual-selector approach ──────────────────────
                // For grouped: data-test-grouped-position-entity-location
                // For standalone: data-test-position-entity-location
                //   → span[data-test-text-highlighter-text-only] for plain text
                ...
                if (locEl) {
                    ...
                    result.debug.push(`Location (experience): "${result.location}"`);
                } else {
                    const locDd = document.querySelector(
                        '[data-test-grouped-position-entity-location], [data-test-position-entity-location]'
                    );
                    if (locDd) {
                        result.location = locDd.textContent.trim();
                        result.debug.push(`Location (experience dd): "${result.location}"`);
                    }
                }
